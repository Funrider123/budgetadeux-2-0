  ...
  createClient: () => ({
    ...
    from: (table) => ({
      ...
      upsert: async (row) => {
        ...
        const keyCol = table === 'couple_state' ? 'couple_code' : 'user_id';
...
/**
 * Place l'app dans l'état "couple connecté", sans passer par l'écran d'auth.
...
async function loginAs(page, state = {}) {
  await page.evaluate((override) => {
    S.auth = Object.assign({
      loggedIn: true, email: 'alex@test.fr', name: 'Alex', profile: 'elle',
