// Synchronisation entre les deux téléphones du couple.
...
/** Construit un état distant plausible (ce que le partenaire aurait déjà poussé). */
...
test.describe('Fusion des ajouts concurrents', () => {
  test('deux dépenses ajoutées en même temps : aucune des deux n\'est perdue', async ({ page }) => {
    ...
    await loginAs(page, {
      ...
      expenses: [{ id: 'local1', type: 'depense', amount: 20, cat: 'courses', who: 'elle', desc: 'Chez moi', date: '2026-08-18', prevision: false }],
    });
    // Le partenaire a poussé SA dépense avant nous, on ne l'a pas encore reçue.
    await seedRemote(page, {
      expenses: [{ id: 'remote1', type: 'depense', amount: 35, cat: 'courses', who: 'lui', desc: 'Chez lui', date: '2026-08-18', prevision: false }],
    });
  ...
  test('la fusion couvre aussi charges, catégories, commerces et projets', async ({ page }) => {
    await openApp(page);
    await loginAs(page, {
      charges: [{ id: 'ch_local', name: 'Loyer', amount: 900, cls: 'besoin', scope: 'commune' }],
      categories: [{ id: 'cat_local', emoji: '🛒', name: 'Courses', budget: 300, cls: 'besoin' }],
      merchants: [{ id: 'm_local', emoji: '🛒', name: 'Carrefour', cat: 'cat_local' }],
      projects: [{ id: 'pr_local', name: 'Voyage', emoji: '🌴', target: 3000, saved: 0, months: 10, mensuel: 300, day: 5, split: { lui: 50, elle: 50 }, cls: 'envie', status: 'actif', history: [] }],
    });
    await seedRemote(page, {
      charges: [{ id: 'ch_remote', name: 'Assurance', amount: 40, cls: 'protection', scope: 'commune' }],
      ...
      projects: [{ id: 'pr_remote', name: 'Cuisine', emoji: '🍳', target: 5000, saved: 0, months: 20, mensuel: 250, day: 5, split: { lui: 50, elle: 50 }, cls: 'envie', status: 'actif', history: [] }],
    });
  ...
  test('un élément déjà connu des deux côtés n\'est pas dupliqué', async ({ page }) => {
    await openApp(page);
    const shared = { id: 'e1', type: 'depense', amount: 20, cat: 'courses', who: 'elle', desc: 'Commune', date: '2026-08-18', prevision: false };
  ...
  test('premier envoi (aucune donnée distante) : rien ne casse', async ({ page }) => {
    await openApp(page);
    await loginAs(page, {
      expenses: [{ id: 'e1', type: 'depense', amount: 20, cat: 'courses', who: 'elle', desc: 'Première', date: '2026-08-18', prevision: false }],
    });
    // Table vide : c'est le tout premier push du couple.
    await page.evaluate(() => { window.__mock.tables.couple_state = []; });
...
test.describe('Mode démonstration', () => {
  ...
  test('ne laisse rien derrière lui dans le stockage local', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('bad2'));
