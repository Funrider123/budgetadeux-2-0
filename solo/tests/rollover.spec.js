...
// de quelqu'un qui tenait ses enveloppes à la main de février à août, avec la formule
    ...
    Object.entries(cfg.depensesParMois).forEach(([off, montants]) => {
      montants.forEach((amount, i) => {
        ...
        S.expenses.push({ id: 'x' + off + '_' + i, amount, date: d.toISOString().slice(0, 10),
                          desc: 'test', cat: 'env', type: 'depense', prevision: false, note: '' });
      });
    });
    save();
  }, cfg);
...
test.describe('Report du solde (excédents et déficits)', () => {
  ...
  test('une catégorie à report ne passe pas par la décision de surplus du point du mois', async ({ page }) => {
    await installer(page, seed({}, 500));
  ...
  test('le report n\'affecte pas la dotation allouée depuis les revenus', async ({ page }) => {
    await installer(page, seed(COURSE, 500));
    // Un gros report ne doit pas gonfler le budget mensuel : ce qu'on verse
    ...
    // ne doit jamais faire croire qu'on a plus à placer en projets.
    const dispo = await page.evaluate(() => { S.settings = { ...S.settings, income:{salary:800, other:0} }; return disponiblePourProjets(); });
  ...
  test('le total "restant pour ce mois" colle à la somme des enveloppes', async ({ page }) => {
    ...
    // histoire : sinon on voit deux montants contradictoires sur le même écran.
...
test.describe('Stabilité du cumul dans le temps', () => {
  test('changer la dotation plus tard ne réécrit pas les mois déjà passés', async ({ page }) => {
    ...
    // On révise son budget à la baisse pour la suite.
