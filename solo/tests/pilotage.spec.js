...
// On doit pouvoir simuler sans que ce soit officiel, puis valider ou tout annuler :
// si « Annuler » ne restaure pas exactement l'état validé, des chiffres deviennent
// officiels sans qu'on l'ait décidé.
const { test, expect } = require('@playwright/test');
const { openApp, loginAs } = require('./helpers');

/** Un budget déjà validé (verrous posés + instantanés enregistrés). */
async function budgetValide(page) {
  await loginAs(page, {
    ...
    projects: [{ id: 'p1', name: 'Voyage', emoji: '🌴', target: 3000, saved: 500, months: 8, mensuel: 300, day: 5, cls: 'envie', status: 'actif', history: [] }],
  });
  await page.evaluate(() => {
    ...
    S.pilotSnapshot = {
      income: { ...S.settings.income },
      charges: S.charges.map(c => ({ id: c.id, amount: c.amount, name: c.name, scope: c.scope })),
      categories: S.categories.map(c => ({ id: c.id, budget: c.budget, name: c.name })),
    };
    S.projectsSnapshot = {};
...
test.describe('Les deux groupes de charges', () => {
  test('se replient et se déplient indépendamment', async ({ page }) => {
    await openApp(page);
    await budgetValide(page);

    const visible = id => page.evaluate(
      i => (document.querySelector('#chgList_' + i) || {}).style.display, id);

    expect(await visible('essentiel')).toBe('none');
    expect(await visible('abonnement')).toBe('none');

    await page.click('[data-togglecharges="essentiel"]');
    expect(await visible('essentiel')).toBe('block');
    expect(await visible('abonnement')).toBe('none'); // l'autre ne bouge pas
  });

  test('chaque groupe affiche son propre total', async ({ page }) => {
    await openApp(page);
    await loginAs(page, {
      charges: [
    ...
    expect(await page.innerText('#chgTotalLbl_essentiel')).toContain('780');
    expect(await page.innerText('#chgTotalLbl_abonnement')).toContain('30');
  });
});
...
test.describe('Détection des changements non validés', () => {
  ...
  test('modifier un revenu est détecté', async ({ page }) => {
    await openApp(page);
    await budgetValide(page);
    await page.evaluate(() => { S.settings.income.salary = 3200; });
    expect(await page.evaluate(() => pilotHasChanges())).toBe(true);
  });
});

test.describe('Annuler mes changements', () => {
  test('restaure budgets, charges ET mensualités, puis reverrouille', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      S.settings.income = { salary: 100, other: 100 };
    });
